...
public class RefreshSessionService {
    ...
    public RefreshSessionService(RefreshSessionRepositorio sessions, UsuarioRepositorio usuarios,
                                 TokenService tokens, Clock clock, AuditService audit) {
    ...
    public Emision iniciar(Usuario usuario, String userAgent, String ip) {
        ...
        audit.registrar("SEGURIDAD", "LOGIN_EXITOSO", "REFRESH_SESSION",
                emision.session().getId().toString(), usuario, null,
    ...
    public Emision rotar(String rawToken, String userAgent, String ip) {
        ...
        if (!actual.getId().toString().equals(verified.jwtId())) throw new InvalidTokenException();
        Instant now = clock.instant();
        if (actual.getUsedAt() != null) {
            sessions.revokeFamily(actual.getFamilyId(), now, "REUSE_DETECTED");
            audit.registrar("SEGURIDAD", "REFRESH_TOKEN_REUSE_DETECTED", "REFRESH_SESSION",
                    actual.getId().toString(), actual.getUsuario(), null,
        ...
        audit.registrar("SEGURIDAD", "REFRESH_EXITOSO", "REFRESH_SESSION",
                nueva.session().getId().toString(), usuario, null,
                Map.of("familyId", actual.getFamilyId().toString(),
    ...
    public void logout(String rawToken) {
        ...
        try {
            ...
            sessions.findByTokenHashForUpdate(hash(rawToken)).ifPresent(session -> {
                ...
                audit.registrar("SEGURIDAD", "LOGOUT", "REFRESH_SESSION",
                        session.getId().toString(), session.getUsuario(), null,
                        Map.of("familyId", session.getFamilyId().toString()));
            });
        } catch (InvalidTokenException ignored) {
            // Invalid cookies are cleared by the HTTP boundary.
        }
    }
    ...
    public record Emision(Usuario usuario, String accessToken, String refreshToken, RefreshSession session) {
    }
}
