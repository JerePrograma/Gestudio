...
$legacyStatePath = Join-Path $stateRoot "quick-tunnel.json"
$secretFile = $null
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            if ($addresses.Count -gt 0) {
                ...
                Pass "DNS Quick Tunnel" $resolved
                return
            }
        }
        catch { }
...
function Stop-TrackedQuickTunnels {
    foreach ($candidateStatePath in @($statePath, $legacyStatePath)) {
        $state = Read-StateFile -Path $candidateStatePath
        if ($null -eq $state -or $null -eq $state.ProcessId) { continue }

        $trackedProcessId = 0
        if (-not [int]::TryParse([string]$state.ProcessId, [ref]$trackedProcessId)) {
            Remove-Item -LiteralPath $candidateStatePath -Force -ErrorAction SilentlyContinue
            continue
        }
        ...
        if (-not $isOwnedTunnel) {
            Write-Host "[WARN] El PID $trackedProcessId no corresponde al Quick Tunnel de Gestudio; no se detuvo." -ForegroundColor Yellow
            Remove-Item -LiteralPath $candidateStatePath -Force -ErrorAction SilentlyContinue
            continue
        }

        Info "Deteniendo Quick Tunnel anterior de Gestudio (PID $trackedProcessId)"
...
function Start-QuickTunnel {
    ...
    Write-TunnelState @stateArguments

    $connectionDeadline = (Get-Date).AddSeconds(180)
    $registered = $false
    ...
    if ($registered) {
        ...
        Write-Host "[WARN] No apareció el mensaje de registro; se continuará con DNS y HTTP." -ForegroundColor Yellow
    }

    Wait-DnsResolution -Uri ([Uri]$origin) -TimeoutSeconds 300
    ...
    [void](Wait-HttpStatus @hiddenCheck)
    ...
    [void](Wait-HttpStatus @backendCheck)
    Pass "Quick Tunnel hacia backend" "HTTP 401 con proxy token y sin sesión"
...
function Assert-GitContract {
    ...
    $topLevel = Invoke-Native -FilePath $git -Arguments @(
        "-C", $repoRoot,
    ...
    $actualRoot = [IO.Path]::GetFullPath($topLevel).TrimEnd($trimCharacters)
    $expectedRoot = $repoRoot.TrimEnd($trimCharacters)
    ...
    if ($branch -ne "main") {
        throw "La rama actual debe ser main; actual=$branch"
    }
...
function Publish-Pages {
    ...
    $previousApiUrl = [Environment]::GetEnvironmentVariable(
        "VITE_API_BASE_URL",
        "Process"
    )
    $previousTimeZone = [Environment]::GetEnvironmentVariable(
        "VITE_APP_TIME_ZONE",
        "Process"
    )
    ...
    try {
        ...
        Invoke-Native -FilePath $npx -Arguments @(
            "--yes", "wrangler",
            "pages", "secret", "bulk", $script:secretFile,
...
function Assert-PublicDeployment {
    $frontendCheck = @{
        Uri = $pagesOriginNormalized
        ExpectedStatuses = @(200)
        TimeoutSeconds = 300
    }
    [void](Wait-HttpStatus @frontendCheck)
    ...
    $profile = Wait-HttpStatus @profileCheck
    ...
    $refresh = Wait-HttpStatus @refreshCheck
    ...
    [void](Wait-HttpStatus @corsCheck)
...
try {
    ...
    Write-TunnelState @finalStateArguments
    ...
    Write-Host ""
    Write-Host "Mantenga el equipo, Docker Desktop y cloudflared encendidos." -ForegroundColor Yellow
}
catch {
    ...
    if (-not [string]::IsNullOrWhiteSpace($secretFile) -and
