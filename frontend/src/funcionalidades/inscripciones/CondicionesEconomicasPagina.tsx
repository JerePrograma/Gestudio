...
const CondicionesEconomicasPagina = () => {
  ...
  if (loading) return <LoadingState message="Cargando condiciones económicas..." />;
  ...
  return (
    <div className="page-container space-y-6">
      <PageHeader
        ...
        description="Historial de costos particulares y bonificaciones copiadas como snapshot."
        count={condiciones.length}
        actions={(
          <Boton onClick={() => navigate("/inscripciones")} className="page-button-secondary">
            Volver
          </Boton>
        )}
      />

      <SectionCard title="Nueva condición" description="Definí desde cuándo rige y qué condición económica aplicar.">
        <form onSubmit={submit} className="space-y-5">
          <div className="form-grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="auth-label">
              ...
              <input
                ...
                onChange={(event) => setCostoParticular(event.target.value)}
              />
            </label>
            ...
            <label className="auth-label">
              ...
              <select
                ...
                onChange={(event) => setBonificacionId(event.target.value)}
              >
                <option value="">Sin bonificación</option>
                {bonificaciones.map((value) => (
          ...
          <div className="form-acciones">
            <Boton type="submit" disabled={saving} className="page-button">
              {saving ? "Guardando..." : "Crear condición"}
            </Boton>
          </div>
        </form>
      </SectionCard>

      <SectionCard title="Historial" description="Condiciones económicas verificadas para esta inscripción.">
        {condiciones.length === 0 && (
          <EmptyState message="No hay condiciones verificadas cargadas." />
        )}
            ...
            <Table className="data-table">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Costo particular</TableHead>
                  <TableHead>Bonificación</TableHead>
                  <TableHead>Porcentaje</TableHead>
                  <TableHead>Valor fijo</TableHead>
              ...
              <TableBody>
                {condiciones.map((value) => (
                  <TableRow key={value.id}>
                    <TableCell>{value.vigenteDesde}</TableCell>
                    <TableCell>{value.costoParticular ?? "Tarifa de disciplina"}</TableCell>
                    <TableCell>{value.bonificacionDescripcion ?? "Sin bonificación"}</TableCell>
                    <TableCell>{value.bonificacionPorcentaje}</TableCell>
                    <TableCell>{value.bonificacionValorFijo}</TableCell>
                    <TableCell>{value.motivo}</TableCell>
                    <TableCell>{value.creadaPorUsername}</TableCell>
                    <TableCell>{value.utilizada ? "Utilizada" : "Sin liquidaciones"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
