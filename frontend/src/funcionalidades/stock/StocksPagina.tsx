...
const StocksPagina = () => {
  ...
  const stocks = useQuery({
    queryKey: queryKeys.stocks(page, PAGE_SIZE),
    queryFn: () => stocksApi.listarStocks(page, PAGE_SIZE),
  });
    ...
    <div className="page-container">
      ...
      <div>
        <Tabla
          ...
          data={stocks.data?.content ?? []}
          getRowKey={(row) => row.id}
          customRender={(row) => [
            <span className="font-semibold" key="nombre">{row.nombre}</span>,
            <span className="numeric-cell block" key="precio">$ {formatMoney(row.precio)}</span>,
            <span className="numeric-cell block" key="stock">{row.stock}</span>,
              ...
              actions={[
                ...
                ...(row.activo
                  ? [{
                      ...
                      onSelect: () => {
                        if (window.confirm(`¿Dar de baja ${row.nombre}?`)) baja.mutate(row.id);
                      },
                    }]
                  : []),
              ]}
      ...
      <PaginationControls
        page={page}
        totalPages={stocks.data?.totalPages ?? 0}
        onPageChange={setPage}
        disabled={stocks.isFetching}
      />
    </div>
  );
};
