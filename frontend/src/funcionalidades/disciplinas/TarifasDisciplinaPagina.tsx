    ...
    <div className="page-container space-y-6">
      ...
      <form onSubmit={submit} className="formulario max-w-5xl mx-auto">
        ...
        <div className="form-grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Field label="Vigente desde" type="date" value={values.vigenteDesde} onChange={(value) => setValues({ ...values, vigenteDesde: value })} />
...
const Field = ({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: "text" | "date" }) => (
  <label className="auth-label">{label}<input className="form-input" type={type} inputMode={type === "text" && label !== "Motivo" ? "decimal" : undefined} min={type === "text" && label !== "Motivo" ? "0" : undefined} step={type === "text" && label !== "Motivo" ? "0.01" : undefined} required value={value} onChange={(event) => onChange(event.target.value)} /></label>
);
