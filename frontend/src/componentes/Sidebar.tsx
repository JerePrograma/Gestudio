    ...
    <>
      ...
      >
        <div className="flex h-[var(--header-height)] items-center gap-3 border-b border-border px-3">
          <Link to="/" className="flex min-w-0 flex-1 items-center gap-3" aria-label="Ir al inicio">
            <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-sm">
              <Sparkles className="size-5" aria-hidden="true" />
            </span>
            {isExpanded && <span className="truncate text-lg font-black tracking-tight text-foreground">LE DANCE</span>}
          </Link>
          <button
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4" aria-label="Navegación principal">
          {isExpanded && <p className="mb-2 px-3 text-[0.6875rem] font-bold uppercase tracking-[0.12em] text-muted-foreground">Menú principal</p>}
